...
class TID2013(Dataset):
    ...
    root_path = "/autofs/unityaccount/ens/am4ip/datasets/tid2013"
    ...
    def __init__(self, transform: Optional[Callable] = None,
        ...
        self.dist_image_paths = glob(expanded_join(
            self.root_path, "distorted_images/*"))
        idx = np.argsort([p.lower() for p in self.dist_image_paths])
        self.dist_image_paths = np.array(self.dist_image_paths)[idx]

        # Get all reference images
        self.ref_image_paths = glob(expanded_join(
            self.root_path, "reference_images/*.BMP"))
    ...
    def __getitem__(self, index: NDArray[int] or int) -> Tuple[Tuple[NDArray, NDArray], NDArray]:
        ...
        if self.do_crop:
            ref_img, dist_img = random_crop(
                ref_img, dist_img, crop_size=self.crop_size)
        ...
        return dist_img, ref_img, np.zeros((3, self.crop_size[0], self.crop_size[0])), np.zeros((3, self.crop_size[0], self.crop_size[0]))
